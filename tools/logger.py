...
class Log:
    ...
    @staticmethod
    def msgDebug(msg):    return logColor.MAGENTA + msg + logColor.ENDC
    @staticmethod
    def msgInfo(msg):     return logColor.BLUE + msg + logColor.ENDC
    @staticmethod
    def msgOk(msg):       return logColor.BOLD + logColor.GREEN + msg + logColor.ENDC
    @staticmethod
    def msgWarn(msg):     return logColor.BOLD + logColor.YELLOW + msg + logColor.ENDC
    @staticmethod
    def msgError(msg):    return logColor.BOLD + logColor.RED + msg + logColor.ENDC
    @staticmethod
    def msgHeader(msg):   return logColor.BOLD + logColor.UNDERLINE + logColor.CYAN + msg + logColor.ENDC
    @staticmethod
    def msgBigHeader(msg):return logColor.BOLD + logColor.CYAN + "===== " + msg.upper() + " =====" + logColor.ENDC
    @staticmethod
    def msgBoldInfo(msg): return logColor.BOLD + logColor.CYAN + msg + logColor.ENDC
    ...
    def toString(self):
        ...
        for msg in self.__LOG:
            resString += self.msgToString(msg)        
            resString += "\n"
    ...
    def msgToString(self, msg):
        ...
        if (k == logMode.TRACE):    resString += "[" + self.msgTrace("TRACE") + "] " + self.msgTrace(m)
        elif (k == logMode.DEBUG):  resString += "[" + self.msgDebug("DEBUG") + "] " + m
        elif (k == logMode.INFO):   resString += "[" +  self.msgInfo("INFO") + "]  " + self.msgInfo(m)
        elif (k == logMode.OK):     resString += "[" +    self.msgOk("OK") + "]    " + self.msgOk(m)
        elif (k == logMode.WARN):   resString += "[" +  self.msgWarn("WARN") + "]  " + self.msgWarn(m)
        elif (k == logMode.ERROR):  resString += "[" + self.msgError("ERROR") + "] " + self.msgError(m)
        else: resString += m.ljust(maxLength)
        
        if (d is not None): resString += msgMode(k, ": ")
